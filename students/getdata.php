<?php
include('session.php');
function getCountForTodayByInterval($table, $dateField, $userIdField, $currentUserId, $intervalHours)
{
    global $con;
    $today = date("Y-m-d");
    ...
    $sql = "SELECT HOUR($dateField) DIV $intervalHours AS interval_slot, COUNT(*) as count 
            FROM $table 
            WHERE DATE($dateField) = '$today' AND $userIdField = $currentUserId 
...
// Likes data for today with 6-hour intervals
$likesToday = getCountForTodayByInterval('posts', 'post_date', 'user_id', $currentUserId, 6);

// Replies data for today with 6-hour intervals
$replyToday = getCountForTodayByInterval('post_replies', 'reply_date', 'user_id', $currentUserId, 6);

// Posts data for today with 6-hour intervals
$postToday = getCountForTodayByInterval('posts', 'post_date', 'user_id', $currentUserId, 6);
